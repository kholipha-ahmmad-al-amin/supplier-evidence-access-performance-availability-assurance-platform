...
Start the service with `PORT=65051 npm start`. It binds to `0.0.0.0` and persists records in `data/access-performance-availability-reviews.json`. Check availability with `GET /health`.

| Condition | Expected response | Operator response |
| --- | --- | --- |
| Invalid availability scope | 422 invalid_input | Correct request body |
